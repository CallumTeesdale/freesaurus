...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = Router::new()
        ...
        // Thesaurus routes (protected by auth middleware)
        ...
        .route("/api/broader/:word", get(routes::thesaurus::get_broader_terms))
        .route("/api/narrower/:word", get(routes::thesaurus::get_narrower_terms))
        .route("/api/related/:word", get(routes::thesaurus::get_related_terms))
        .route("/api/definition/:word", get(routes::thesaurus::get_definition))
    ...
    // Get the host from environment variables or use default
    ...
    let host = match std::env::var("HOST") {
        Ok(host_str) => {
            if host_str == "0.0.0.0" {
                ...
                host_str.parse().unwrap_or_else(|_| std::net::IpAddr::V4(std::net::Ipv4Addr::new(0, 0, 0, 0)))
            }
        },
